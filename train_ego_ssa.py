...
FLAGS = flags.FLAGS
model_save_dir = './models/ego_1400_6_300_0.0001_decay_ssa__lr001_5c'
...
def run_training():
    ...
    with tf.Graph().as_default():
        ...
        with tf.control_dependencies(update_ops):
            ...
            if (step_count > FLAGS.max_steps * 0.8 ) and (step_count !=0):
            #     #153个step是一个epoch，因为一个训练集有 video num train / batch size =153个batch
            #     flag_ssa=1
            # if (((step_count+1) % 15 % 5 )==0):
            #     flag_ssa=0
            # if flag_ssa==1:# or step_count==0:
            #     #SSA 
                print("-----start culc ssa_r_f-----")
                feat_r=rgbends['Mixed_5c']
                feat_f=flowends['Mixed_5c']
                #计算ssr loss (rgb,flow)
                ssa_r_f=tf.multiply(frp(rgb_loss,flow_loss),normsq(feat_r,feat_f))
                ssa_f_r=tf.multiply(frp(flow_loss,rgb_loss),normsq(feat_r,feat_f))
                print(ssa_r_f)
                print("_____----------_______")
                #SSA END
                rgb_grads = opt_rgb.compute_gradients(tf.add(rgb_loss,tf.multiply(ssa_r_f,0.05)), var_list=rgb_variable_list)
                flow_grads = opt_flow.compute_gradients(tf.add(flow_loss,tf.multiply(ssa_f_r,0.05)), var_list=flow_variable_list)
            else:
    ...
    if ckpt and ckpt.model_checkpoint_path:
        ...
        print("load complete!")
    print("M2+")
    train_writer = tf.summary.FileWriter('./visual_logs/train_ego_1400_6_300_0.0001_decay_ssa__lr001_5c', sess.graph)
    test_writer = tf.summary.FileWriter('./visual_logs/test_ego_1400_6_300_0.0001_decay_ssa__lr001_5c', sess.graph)
    for step in xrange(FLAGS.max_steps):
        ...
        if step % 94 == 0 or (step + 1) == FLAGS.max_steps:
            ...
            fileObject = open('ego_1400_6_300_0.0001_decay_ssa__lr001_5c.txt', 'a')
        ...
        if (step + 1) == FLAGS.max_steps:
            saver.save(sess, os.path.join(model_save_dir, 'i3d_ego_1400_6_300_0.0001_decay_ssa__lr001_5c'), global_step=step)
    
            pred_result=[]
            for sample in total_pred:
                pred_result.append(np.argmax(sample))
            fpred = open('pred_result_ego_1400_6_300_0.0001_decay_ssa__lr001_5c.txt','a')
            flabel = open('label_result_ego_1400_6_300_0.0001_decay_ssa__lr001_5c.txt','a')
            ...
            plt.savefig('heatmap_ego_1400_6_300_0.0001_decay_ssa__lr001_5c.jpg')
